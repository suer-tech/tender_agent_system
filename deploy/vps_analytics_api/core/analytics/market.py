    ...
    with eis_analytics.conn() as c:
        ...
        row = c.execute(f"""
            SELECT COUNT(*) AS contracts_count,
                   COALESCE(SUM(contract_price), 0) AS total_sum,
                   AVG(contract_price) AS avg_price
            FROM (
                SELECT DISTINCT c.reg_num, c.contract_price
                FROM contracts c
        ...
        supp_shares = c.execute(f"""
            SELECT supplier_inn, SUM(contract_price) AS s
            FROM (
                SELECT DISTINCT c.reg_num, c.supplier_inn, c.contract_price
                FROM contracts c
                JOIN contract_items ci ON ci.reg_num = c.reg_num
                WHERE {okpd2_sql} AND {period_sql} AND {region_sql}
...
def top_sectors(region_code: str | None, from_date: str, to_date: str,
    ...
    with eis_analytics.conn() as c:
        ...
        total_row = c.execute(f"""
            SELECT COALESCE(SUM(contract_price), 0) AS total
            FROM (
                SELECT DISTINCT c.reg_num, c.contract_price
                FROM contracts c
                JOIN contract_items ci ON ci.reg_num = c.reg_num
        ...
        rows = c.execute(f"""
            SELECT prefix,
                   ...
                   COALESCE(SUM(contract_price), 0) AS total_sum
            FROM (
                SELECT DISTINCT c.reg_num, substr(ci.okpd2_code, 1, 2) AS prefix,
                                c.contract_price
                FROM contracts c
                JOIN contract_items ci ON ci.reg_num = c.reg_num
                WHERE {region_sql} AND {period_sql} AND ci.okpd2_code IS NOT NULL
    ...
    with eis_analytics.conn() as c:
        ...
        ts_rows = c.execute(f"""
            SELECT month,
                   ...
                   COALESCE(SUM(contract_price), 0) AS total_sum
            FROM (
                SELECT DISTINCT c.reg_num, substr(c.sign_date, 1, 7) AS month,
                                c.contract_price
                FROM contracts c
                JOIN contract_items ci ON ci.reg_num = c.reg_num
                WHERE ci.okpd2_code = ? AND {region_sql} AND {period_sql}
    ...
    with eis_analytics.conn() as c:
        rows = c.execute(f"""
            SELECT customer_inn AS inn, MAX(customer_name) AS name,
                   ...
                   COALESCE(SUM(contract_price), 0) AS total_sum
            FROM (
                SELECT DISTINCT c.reg_num, c.customer_inn, c.customer_name,
                                c.contract_price
                FROM contracts c
                JOIN contract_items ci ON ci.reg_num = c.reg_num
                WHERE {okpd2_sql} AND {region_sql} AND {period_sql}
    ...
    with eis_analytics.conn() as c:
        rows = c.execute(f"""
            SELECT supplier_inn AS inn, MAX(supplier_name) AS name,
                   ...
                   COALESCE(SUM(contract_price), 0) AS total_sum
            FROM (
                SELECT DISTINCT c.reg_num, c.supplier_inn, c.supplier_name,
                                c.contract_price
                FROM contracts c
                JOIN contract_items ci ON ci.reg_num = c.reg_num
                WHERE {okpd2_sql} AND {region_sql} AND {period_sql}
